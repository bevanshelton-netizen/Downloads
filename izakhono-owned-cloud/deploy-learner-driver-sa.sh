...
SOURCE_DIR="learner-driver-sa-v6-live"
HOSTNAME="${LEARNER_DRIVER_SA_HOSTNAME:-learnerdriver.domains.izakhonoafrica.co.za}"
REVISION="${1:-main}"
...
REPORT="$REPORT_DIR/learner-driver-sa.json"
...
if [[ "$REPO_URL" != http://127.0.0.1:8860/git/* ]] && [ "${ALLOW_EXTERNAL_SOURCE:-0}" != "1" ]; then
  fail "External source refused. Learner Driver SA must deploy from IZAKHONO CODE."
fi
...
test -f "$RELEASE/index.html" || fail "Learner Driver SA index.html missing."
test -f "$RELEASE/server.mjs" || fail "Learner Driver SA independent engine missing."
...
node -e 'const x=JSON.parse(process.argv[1]);if(x.ok!==true||x.service!=="learner-driver-sa"||x.runtime!=="izakhono-owned"||x.engine_independent!==true)process.exit(2)' "$HEALTH"
curl -fsS -H "Host: $HOSTNAME" "$PROXY_URL/" | grep -Fq "Learner Driver SA"

EDGE="NOT_RUNNING"
if systemctl is-active --quiet izakhono-edge-node 2>/dev/null; then
  if curl -fsS -H "Host: $HOSTNAME" "$EDGE_URL/health" >/tmp/learner-driver-sa-edge-health.json 2>/dev/null; then
    node -e 'const x=require("/tmp/learner-driver-sa-edge-health.json");if(x.ok!==true||x.service!=="learner-driver-sa")process.exit(2)'
    EDGE="VERIFIED"
  else
...
if curl -fsS --max-time 8 "https://$HOSTNAME/health" >/tmp/learner-driver-sa-public-health.json 2>/dev/null; then
  if node -e 'const x=require("/tmp/learner-driver-sa-public-health.json");if(x.ok!==true||x.service!=="learner-driver-sa"||x.runtime!=="izakhono-owned")process.exit(2)' 2>/dev/null; then
...
fs.writeFileSync(path,JSON.stringify({
  ...
  engine:"learner-driver-sa",
  engine_independent:true,
  edge,
  public_https:publicHttps,
  public_live_verified:publicHttps==="VERIFIED",
  external_resilience:"https://bevanshelton-netizen.github.io/Downloads/learner-driver-sa-v6-live/",
...
cat <<EOF
LEARNER DRIVER SA OWNED DEPLOYMENT
APP=$APP
HOSTNAME=$HOSTNAME
REVISION=$RESOLVED
...
PUBLIC_LIVE_VERIFIED=$([ "$PUBLIC_HTTPS" = "VERIFIED" ] && echo YES || echo NO)
EXTERNAL_RESILIENCE=PRESERVED
VERCEL_REQUIRED=NO
RECEIPT=$REPORT
EOF
