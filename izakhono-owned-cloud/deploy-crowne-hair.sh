...
set -euo pipefail
APP="crowne-hair"
SOURCE_DIR="crowne-hair"
HOSTNAME="${CROWNE_HAIR_HOSTNAME:-hair.domains.izakhonoafrica.co.za}"
REVISION="${1:-main}"
...
REPORT="$REPORT_DIR/crowne-hair.json"
...
const [app,hostname,releasePath,envFile]=process.argv.slice(2);process.stdout.write(JSON.stringify({app,hostname,releasePath,command:["node","server.mjs"],envFile:envFile||null,healthPath:"/health"}));
NODE
)"
