...
for cmd in git curl node tar awk grep; do need "$cmd"; done
...
test -f "$RELEASE/marketplace-pay.js" || fail "LegacyMart marketplace-pay.js missing"
...
grep -q '^PAYFAST_MODE=sandbox$' "$APP_ENV" || fail "Legacy PayFast mode must remain sandbox on the reference pilot"
grep -q '^CHECKOUT_ENABLED=false$' "$APP_ENV" || fail "Legacy ebook checkout must remain disabled on the reference pilot"
...
TMP_REPORT="$(mktemp)"
node - "$TMP_REPORT" "$HOSTNAME" "$RESOLVED" "$DEPLOYMENT_ID" "$EDGE" "$PUBLIC_HTTPS" "$REFERENCE_ENABLED" <<'NODE'
const fs=require("fs");
const [path,hostname,revision,deploymentId,edge,publicHttps,referenceEnabled]=process.argv.slice(2);
fs.writeFileSync(path,JSON.stringify({
  ...
  reference_checkout_enabled:referenceEnabled==="true",
...
REFERENCE_CHECKOUT_ENABLED=$REFERENCE_ENABLED
