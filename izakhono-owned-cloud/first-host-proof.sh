...
probe_json PACKAGE http://127.0.0.1:8910/health "IZAKHONO PACKAGE NODE"
...
probe_json FAILOVER http://127.0.0.1:8920/health "IZAKHONO FAILOVER NODE"
probe_json DNS http://127.0.0.1:8900/health "IZAKHONO DNS NODE"
DNS_HEALTH="$(curl -fsS http://127.0.0.1:8900/health)"
node -e 'const x=JSON.parse(process.argv[1]); if(x.authoritative!==true||x.recursive!==false)process.exit(2)' "$DNS_HEALTH" || fail "DNS NODE authoritative/non-recursive safety posture invalid"
ok "DNS NODE authoritative and recursion disabled"
FAILOVER_HEALTH="$(curl -fsS http://127.0.0.1:8920/health)"
node -e 'const x=JSON.parse(process.argv[1]); if(x.automaticPromotion!==false||x.splitBrainProtection!=="fence-required")process.exit(2)' "$FAILOVER_HEALTH" || fail "FAILOVER safety posture invalid"
ok "FAILOVER automatic promotion disabled and fencing required"
