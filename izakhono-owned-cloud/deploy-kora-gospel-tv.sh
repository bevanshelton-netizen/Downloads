...
REPORT="$REPORT_DIR/kora-gospel-tv.json"
...
if systemctl is-active --quiet izakhono-edge-node 2>/dev/null; then
  if curl -fsS -H "Host: $HOSTNAME" "$EDGE_URL/health" >/tmp/kora-gospel-tv-edge-health.json 2>/dev/null; then
    node -e 'const x=require("/tmp/kora-gospel-tv-edge-health.json");if(x.ok!==true||x.service!=="kora-gospel-tv")process.exit(2)'
    EDGE="VERIFIED"
  else
