...
APP="kora-kids"
SOURCE_DIR="ports/kora-kids"
HOSTNAME="${KORA_KIDS_HOSTNAME:-korakids.domains.izakhonoafrica.co.za}"
REVISION="${1:-main}"
...
REPORT="$REPORT_DIR/kora-kids.json"
...
if [[ "$REPO_URL" != http://127.0.0.1:8860/git/* ]] && [ "${ALLOW_EXTERNAL_SOURCE:-0}" != "1" ]; then
  fail "External source refused. KORA KIDS must deploy from IZAKHONO CODE."
fi
...
test -f "$RELEASE/server.mjs" || fail "KORA KIDS server.mjs missing."
test -f "$RELEASE/index.html" || fail "KORA KIDS index.html missing."
...
node -e 'const x=JSON.parse(process.argv[1]);if(x.ok!==true||x.service!=="kora-kids"||x.runtime!=="izakhono-owned")process.exit(2)' "$HEALTH"
curl -fsS -H "Host: $HOSTNAME" "$PROXY_URL/" | grep -q "TUMI"
curl -fsS -H "Host: $HOSTNAME" "$PROXY_URL/" | grep -q "KORA KIDS"

EDGE="NOT_RUNNING"
if systemctl is-active --quiet izakhono-edge-node 2>/dev/null; then
  if curl -fsS -H "Host: $HOSTNAME" "$EDGE_URL/health" >/tmp/kora-kids-edge-health.json 2>/dev/null; then
    node -e 'const x=require("/tmp/kora-kids-edge-health.json");if(x.ok!==true||x.service!=="kora-kids")process.exit(2)'
    EDGE="VERIFIED"
  else
...
if curl -fsS --max-time 8 "https://$HOSTNAME/health" >/tmp/kora-kids-public-health.json 2>/dev/null; then
  if node -e 'const x=require("/tmp/kora-kids-public-health.json");if(x.ok!==true||x.service!=="kora-kids")process.exit(2)' 2>/dev/null; then
...
fs.writeFileSync(path,JSON.stringify({
  schema:"izakhono.kora-kids-deployment/v1",
  app:"kora-kids",
