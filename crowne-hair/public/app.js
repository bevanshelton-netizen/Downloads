...
function addToBag(sku){const p=products.find(x=>x.sku===sku);if(!p)return;cart=[p];renderBag();const apparel=p.sku==="CRN-GOLF-LADIES";document.querySelectorAll(".hair-only").forEach(el=>el.hidden=apparel);$("#preferredSizeWrap").hidden=!apparel;if(apparel){$("#preferredLength").value="";$("#preferredTexture").value="";$("#preferredColour").value="Cream / Black / Gold";paymentNote.textContent="Select your size. Garment specification, stock and final price are confirmed before payment."}else{$("#preferredSize").value="";paymentNote.textContent="One style per reservation during launch. We confirm stock, exact specification, delivery and final price before payment."}openBag()}
