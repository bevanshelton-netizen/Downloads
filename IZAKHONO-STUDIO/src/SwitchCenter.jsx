...
const SOURCE = new Set(['psd','ai','indd','aep','prproj','eps']);
  ...
  return (
    <section className="switch-center" id="switch-center">
      <div className="suite-section-copy">
        ...
        <p>Bring existing creative files into one migration inventory. Originals are preserved, common formats are accepted first, and proprietary source files are flagged for compatible conversion rather than silently discarded.</p>
      </div>
        ...
        <div className="switch-drop">
          <label className="suite-upload switch-upload">
            ...
            <input
              type="file"
              multiple
              accept=".png,.jpg,.jpeg,.webp,.svg,.pdf,.mp4,.mov,.wav,.mp3,.m4a,.psd,.ai,.indd,.aep,.prproj,.eps"
          ...
          <small className="switch-note">This does not claim full native import of proprietary formats yet. It creates a safe, auditable migration pack while those importers are built.</small>
        </div>
        ...
        <div className="switch-inventory">
          {inventory.length === 0 ? (
            <div className="switch-empty">
              ...
              <span>Add files to see exactly what can move directly and what needs a conversion path.</span>
            </div>
          ) : inventory.map((item) => (
            <div className="switch-file" key={`${item.file.name}-${item.file.size}-${item.file.lastModified}`}>
              <div className="switch-file-main">
                <strong>{item.file.name}</strong>
                <span>{(item.file.size / (1024 * 1024)).toFixed(item.file.size > 1024 * 1024 ? 1 : 2)} MB • .{ext(item.file.name) || 'file'}</span>
              </div>
