...
async function sharePlatform(button){
  ...
  try{
    if(navigator.share){await navigator.share({title,text:description,url});return;}
    if(navigator.clipboard?.writeText){await navigator.clipboard.writeText(url);if(button){const old=button.textContent;button.textContent='✓ Link copied';setTimeout(()=>button.textContent=old,1800);}return;}
...
function mount(){
  ...
  b.type='button';b.setAttribute('data-izakhono-share','1');b.setAttribute('aria-label','Share this platform');b.textContent='↗ Share';
  ...
  b.addEventListener('click',()=>sharePlatform(b));
  document.body.appendChild(b);
  document.querySelectorAll('[data-share-platform]').forEach(el=>el.addEventListener('click',()=>sharePlatform(el)));
}
