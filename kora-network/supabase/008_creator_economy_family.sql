...
create table if not exists public.creator_revenue_allocations (
  ...
  created_at timestamptz not null default now(),
  unique(revenue_event_id, creator_id)
);
...
create policy "creator applicant updates own pending" on public.creator_applications
for update using (user_id = auth.uid() and status in ('submitted','reviewing'))
...
create policy "owner creates payout profile" on public.payout_profiles
for insert to authenticated with check (owner_id = auth.uid());
create policy "owner updates payout profile" on public.payout_profiles
for update using (owner_id = auth.uid()) with check (owner_id = auth.uid());
create policy "staff manages payout profiles" on public.payout_profiles
for all using (public.is_staff()) with check (public.is_staff());

create policy "owner manages family profiles" on public.viewer_profiles
...
-- Service-only allocation from an already-cleared revenue event to a creator wallet.
-- The client cannot choose the contractual share percentage.
...
begin
  ...
  where creator_id = p_creator_id and status = 'accepted'
  ...
  if v_creator_amount > 0 then
    ...
    values(v_wallet, 'credit', v_creator_amount, 'Creator revenue share', 'revenue_event', p_revenue_event_id::text)
    returning id into v_ledger;
  end if;

  insert into public.creator_revenue_allocations(
    revenue_event_id, creator_id, deal_id, eligible_amount, creator_amount, platform_amount, ledger_entry_id
  ) values (
    p_revenue_event_id, p_creator_id, v_deal_id, p_eligible_amount, v_creator_amount, v_platform_amount, v_ledger
