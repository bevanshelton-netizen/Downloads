...
create policy "published artist profiles are public" on public.artist_profiles for select using (is_published = true or owner_id = auth.uid() or public.is_staff());
create policy "staff manages artist profiles" on public.artist_profiles for all using (public.is_staff()) with check (public.is_staff());
create index if not exists artist_profiles_published_genre_idx on public.artist_profiles(is_published, primary_genre, display_name);
create index if not exists artist_profiles_country_idx on public.artist_profiles(is_published, country_code, display_name);
grant select on public.artist_profiles to anon, authenticated;
grant all on public.artist_profiles to service_role;
update public.platform_release_state set schema_version = greatest(schema_version, 18), updated_at = now() where singleton = true;
