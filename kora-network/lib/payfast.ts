...
function payFastApiSignature(fields: Record<string, string>, passphrase: string) {
  const signed = { ...fields, passphrase };
  const pairs = Object.entries(signed)
    .filter(([, value]) => value !== '')
...
function checkoutConfig(requirePassphrase = false) {
  ...
  if (!merchantId || !merchantKey || !appUrl || (requirePassphrase && !passphrase)) throw new Error('Missing PayFast checkout configuration');
  return { merchantId, merchantKey, passphrase, appUrl: appUrl.replace(/\/$/, '') };
}

function actionUrl() {
  return process.env.PAYFAST_SANDBOX === 'false' ? 'https://www.payfast.co.za/eng/process' : 'https://sandbox.payfast.co.za/eng/process';
}
...
export function buildPurchaseCheckout(input: { orderId: string; email: string; title: string; amount: number }) {
...
export async function cancelPayFastSubscription(token: string) {
  const merchantId = process.env.PAYFAST_MERCHANT_ID;
  const passphrase = process.env.PAYFAST_PASSPHRASE;
  if (!merchantId || !passphrase || !token) throw new Error('Missing PayFast recurring configuration');

  const timestamp = new Date().toISOString().replace(/\.\d{3}Z$/, '+00:00');
  const signatureFields = { 'merchant-id': merchantId, timestamp, version: 'v1' };
  const signature = payFastApiSignature(signatureFields, passphrase);
  ...
  if (!response.ok || payload?.status !== 'success' || payload?.data?.response !== true) {
    throw new Error(payload?.data?.message || payload?.message || 'PayFast could not cancel the subscription');
  }
  return true;
}
...
export async function validateItn(fields: Record<string, string>) {
  ...
  const validateUrl = process.env.PAYFAST_SANDBOX === 'false' ? 'https://www.payfast.co.za/eng/query/validate' : 'https://sandbox.payfast.co.za/eng/query/validate';
