...
export async function cancelPayFastSubscription(token: string) {
  ...
  const payload = await response.json().catch(() => null) as { status?: string; data?: { response?: boolean } } | null;
  if (payload && payload.status && payload.status !== 'success') throw new Error('PayFast rejected the cancellation');
  return true;
}
