...
Use only for a brand-new Supabase production project. It requires the exact confirmation `BOOTSTRAP FRESH KORA DATABASE`, checks that `public.profiles` does not already exist, applies the fresh installer and migrations 006–013 in order, and verifies schema version 13.
...
Runs migration inventory validation, TypeScript, production environment validation and the full Next.js build. This is a public-launch preflight: all legal, regulatory, child-safety, payout, backup and incident-response sign-offs must be true.
...
Private beta may keep PayFast sandbox enabled and may keep public-launch approval gates false. The database release state remains fail-closed, so anonymous public visitors stay on the coming-soon experience until an administrator deliberately enables public launch from `/admin/launch`.
...
4. run the production smoke test with readiness expected to be true;
5. open `/admin/launch` and enable public launch only after the server-side readiness gate is green.
