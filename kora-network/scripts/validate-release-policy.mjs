...
check('Readiness requires schema 14 or newer', readiness.includes('schemaCurrent: release.schema_version >= 14'));
...
const publicPrivilegedName = /NEXT_PUBLIC_[A-Z0-9_]*(?:SECRET|SERVICE_ROLE|PASSPHRASE|STREAM_TOKEN|INTERNAL_API_SECRET)/;
let leakedSecret = null;
let publicPrivileged = null;
for (const file of scanFiles) {
  const text = fs.readFileSync(file, 'utf8');
  if (!leakedSecret && (longSupabaseSecret.test(text) || supabasePersonalToken.test(text))) leakedSecret = path.relative(repoRoot, file);
  if (!publicPrivileged && publicPrivilegedName.test(text)) publicPrivileged = path.relative(repoRoot, file);
}
