...
const preflightIndex = workflow.indexOf('npm run preflight:production');
const dbIndex = workflow.indexOf('npm run ensure:production-db');
const adminIndex = workflow.indexOf('npm run ensure:admin');
const deployIndex = workflow.indexOf('vercel@latest deploy --prebuilt --prod');
const smokeIndex = workflow.indexOf('npm run smoke:production');
...
check('Database ensure carries ticket settlements before ALLEGRO handoff', db.indexOf('019_ticket_settlements.sql') >= 0 && db.indexOf('020_allegro_video_handoff.sql') > db.indexOf('019_ticket_settlements.sql'));
check('Database ensure carries ALLEGRO handoff before Tour2Screen', db.indexOf('020_allegro_video_handoff.sql') >= 0 && db.indexOf('021_tour2screen.sql') > db.indexOf('020_allegro_video_handoff.sql'));
check('Database ensure carries Tour2Screen before music-screen gate', db.indexOf('021_tour2screen.sql') >= 0 && db.indexOf('022_music_screen_release_gate.sql') > db.indexOf('021_tour2screen.sql'));
check('Database ensure carries music-screen gate before iKhokha settlement', db.indexOf('022_music_screen_release_gate.sql') >= 0 && db.indexOf('023_ikhokha_payments.sql') > db.indexOf('022_music_screen_release_gate.sql'));
...
if (failures.length) {
  console.error('\nActivation orchestration violations:');
