...
export default async function Studio({ searchParams }: { searchParams: Promise<{ created?: string }> }) {
  ...
  let offeredDeals = 0;
  let ticketSettlementCount = 0;
  if (creator) {
    const [productionResult, dealResult, ticketResult] = await Promise.all([
      ...
      supabase.from('ticket_settlements').select('id', { count: 'exact', head: true }).eq('owner_id', user.id),
    ]);
    ...
    ticketSettlementCount = ticketResult.count ?? 0;
  }
  ...
  return (
    <main>
      <section className="subHero">
        ...
        <h1>Publish. Perform. Sell. Monetise.</h1>
        <p>Protected production console for content, rights, live events, ticket sales, moderation, analytics and earnings.</p>
      ...
      <section className="dashboard">
        ...
        <div className="dashMain">
          ...
          <div className="kpis">
            ...
            <div><small>Ticket settlements</small><b>{creator ? ticketSettlementCount : 0}</b></div>
