...
export default async function ArtistPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    const supabase = await createClient();
    const result = await supabase.from('artist_profiles').select('id,live_application_id,display_name,country_code,primary_genre,bio,portfolio_url,public_booking_email,website_url,social_url').eq('slug', slug).eq('is_published', true).maybeSingle();
    artist = result.data;
    if (artist) {
      const eventResult = await supabase.from('ticket_events').select('id,title,slug,starts_at,event_mode,status,sales_enabled,venue_city').eq('live_application_id', artist.live_application_id).in('status', ['published','postponed','completed']).order('starts_at');
      ...
      <section className={styles.profile}>
        <div className={styles.profileHero}>
          <div>
            ...
            <h1>{artist.display_name}</h1><p>{artist.bio}</p>
