...
export default async function AdvertiserReports() {
  ...
  return <main>
    <section className="subHero"><div className="eyebrow">KORA FOR BRANDS</div><h1>Campaign reporting.</h1><p>See aggregate delivery, controlled media spend and verified engagement without receiving viewer identities or household-profile data.</p><div className="actions"><Link className="secondary" href="/advertiser">Campaigns</Link><Link className="secondary" href="/advertiser/creatives">Creatives</Link></div></section>
    <section className="dashMain">
      {reports.length ? reports.map(({campaign,summary}) => {
        ...
        return <div className="panel" key={campaign.id}>
          <div className="sectionHead"><div><h3>{campaign.name}</h3><p>{campaign.status} • budget R{Number(campaign.budget).toFixed(2)} • reward reserve R{Number(campaign.reward_pool).toFixed(2)} • media CPM R{Number(summary?.media_cpm || 0).toFixed(2)}</p></div></div>
          <div className="kpis"><div><small>Deliveries</small><b>{Number(summary?.deliveries || 0).toLocaleString('en-ZA')}</b></div><div><small>Impressions</small><b>{impressions.toLocaleString('en-ZA')}</b></div><div><small>Media spend reserved</small><b>R{Number(summary?.media_spend || 0).toFixed(2)}</b></div></div>
