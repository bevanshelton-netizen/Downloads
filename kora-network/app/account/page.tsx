...
export default async function Account({
  ...
  return (
    <main>
      ...
      {(error || payment || purchase || payout || required) ? <section><div className="panel">
      ...
      <section className="grid three">
        ...
        <article className="panel">
          ...
          <p>Minimum payout R100. A request immediately reserves the amount to prevent double spending.</p>
      ...
      <section className="grid two">
        <div className="panel"><h3>Recent memberships</h3>{(subscriptions ?? []).length ? (subscriptions ?? []).map((sub) => <div className="productionRow" key={sub.id}><strong>{sub.plan_code}</strong><span>{sub.status}{sub.current_period_end ? ` • through ${new Date(sub.current_period_end).toLocaleDateString('en-ZA')}` : ''}</span></div>) : <p>No subscription yet.</p>}</div>
