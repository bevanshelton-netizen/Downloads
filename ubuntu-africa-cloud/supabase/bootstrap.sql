-- Ubuntu Africa Cloud — single-run bootstrap for a NEW Supabase project
-- This combines schema.sql, phase3.sql and final.sql.
-- Run once in Supabase SQL Editor on an empty/new project.

create extension if not exists pgcrypto;

create type public.app_role as enum (
  'super_admin','support_admin','tenant_owner','tenant_admin','tenant_member'
);

create type public.project_status as enum (
  'draft','submitted','approved','published','suspended'
);
...
-- AFTER registering the owner account, run this separately with the real email:
