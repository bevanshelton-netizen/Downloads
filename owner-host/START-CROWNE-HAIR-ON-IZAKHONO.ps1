...
if ($LASTEXITCODE -ne 0) {
  throw "Crowne by Netty owned-infrastructure deployment failed."
}
...
if ($edgeCode -eq 20) {
  Write-Host ""
  Write-Host "CROWNE BY NETTY: runtime is deployed and the shared owned DNS zone is installed." -ForegroundColor Green
...
$public = Invoke-WebRequest -UseBasicParsing -TimeoutSec 12 "https://$Hostname/health"
$health = $public.Content | ConvertFrom-Json
if ($health.ok -ne $true -or $health.service -ne "crowne-hair") {
  throw "Public Crowne by Netty health verification failed."
}

Write-Host ""
Write-Host "CROWNE BY NETTY: PUBLIC HTTPS LIVE AND VERIFIED" -ForegroundColor Green
