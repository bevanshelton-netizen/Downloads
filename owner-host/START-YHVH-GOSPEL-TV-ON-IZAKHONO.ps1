...
export YHVH_GOSPEL_TV_HOSTNAME='$escapedHost'
bash izakhono-owned-cloud/deploy-yhvh-gospel-tv.sh main
"@
...
try {
  ...
  if ($health.ok -eq $true -and $health.service -eq "yhvh-gospel-tv") {
...
$public = Invoke-WebRequest -UseBasicParsing -TimeoutSec 12 "https://$Hostname/health"
$health = $public.Content | ConvertFrom-Json
if ($health.ok -ne $true -or $health.service -ne "yhvh-gospel-tv") { throw "Public YHVH GOSPEL TV health verification failed." }
