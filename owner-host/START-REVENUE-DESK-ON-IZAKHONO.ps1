...
try {
  ...
  if ($health.ok -eq $true -and $health.service -eq "izakhono-revenue-desk") {
...
$public = Invoke-WebRequest -UseBasicParsing -TimeoutSec 12 "https://$Hostname/health"
$health = $public.Content | ConvertFrom-Json
if ($health.ok -ne $true -or $health.service -ne "izakhono-revenue-desk") {
  throw "Public Revenue Desk health verification failed."
}

Write-Host ""
Write-Host "IZAKHONO REVENUE DESK: PUBLIC HTTPS LIVE AND VERIFIED" -ForegroundColor Green
