...
<section id="landing">
  <div class="hero"><div class="heroInner"><div class="eyebrow">Brief → 3 directions → verified live site</div><h1>Build a serious website in minutes.</h1><p>Describe the business once. WebStart creates three professional directions, generates the coded site, runs automated QA and publishes only after public verification.</p><div class="heroActions"><button class="btn primary" onclick="showAuth()">Build my website</button><button class="btn ghost" onclick="document.getElementById('pricing').scrollIntoView()">See packages</button></div></div></div>
  <div id="pricing" class="packages">
    ...
    <article class="pkg"><h3>Business</h3><div class="price">R2,499</div><div class="sub">once-off + R199/month</div><ul><li>5–8 page structure</li><li>SEO-ready copy</li><li>Gallery, forms & analytics-ready</li></ul><button class="btn primary" onclick="startPackage('business')">Choose Business</button></article>
    ...
    <div class="grid2">
      ...
      <section class="card">
        ...
        <div id="editor" class="hidden">
          ...
          <div class="factory"><div class="factoryHead"><div><div class="eyebrow" style="color:#087f89">FAST FACTORY</div><h3>Describe the business once</h3><p class="muted" style="margin:6px 0">Include what the business does, who it serves, location, differentiators and any must-have services. WebStart handles the structure and design.</p></div><span id="qaBadge" class="qaBadge hidden">QA READY</span></div><div class="field full"><label>Business brief</label><textarea id="factoryBrief" placeholder="Example: We are a Johannesburg construction company specialising in renovations, maintenance and commercial projects. We want to look established, professional and easy to contact."></textarea></div><div id="directionCards" class="directions"></div></div>
          <div class="fields">
...
<footer class="foot"><strong>IZAKHONO AFRICA (PTY) LTD</strong> · Reg. 2022/699539/07 · Merchant responsible for WebStart services.<br><a class="link" target="_blank" rel="noopener" href="https://yfawrenhudjomhnglfhq.supabase.co/functions/v1/webstart-policies?doc=terms">Terms & Conditions</a> · <a class="link" target="_blank" rel="noopener" href="https://yfawrenhudjomhnglfhq.supabase.co/functions/v1/webstart-policies?doc=privacy">Privacy</a> · <a class="link" target="_blank" rel="noopener" href="https://yfawrenhudjomhnglfhq.supabase.co/functions/v1/webstart-policies?doc=refund">Refunds & Cancellations</a></footer>
...
function startPackage(p){preferredPackage=p;window.IZGrowth?.track("cta_click",{cta:"choose_package",plan:p,page:"webstart"});showAuth()}
...
async function submitAuth(){
  ...
  try{
    ...
    if(d.access_token){session=d;localStorage.setItem("webstart_session",JSON.stringify(d));window.IZGrowth?.track("signup",{page:"webstart",audience:"business_owner"});await enterApp()}
...
async function generateSite(){
  ...
  selected=d.site;$("preview").srcdoc=d.html;renderDirections(selected.design_directions||[],selected.selected_direction);const qb=$("qaBadge");qb.textContent="QA "+(d.qa?.score||100)+"%";show(qb,true);window.IZGrowth?.track("creative_created",{product:"website",plan:$("package").value,page:"webstart"});message("Website generated. Automated QA passed at "+(d.qa?.score||100)+"%.");await loadSites()
}
